(function(){
	...
	var CardsCtrl = function($stateParams, $state, $filter, $log, $mdDialog, CardsService, VehicleInformationService){
		var vm = this;
	...
	CardsCtrl.$inject = ['$stateParams', '$state', '$filter', '$log', '$mdDialog', 'CardsService', 'VehicleInformationService'];
	...
	app.config(function($stateProvider) {
        ...
        .state('cards', {
            url: '/cards',
            parent: 'dashboard',
            templateUrl: 'views/pages/dashboard/cards/cards.html?v='+window.appVersion,
        ...
        .state('cards.view', {
            url: '/cards/:cardID',
            parent: 'dashboard',
            templateUrl: 'views/pages/dashboard/cards/view-card.html?v='+window.appVersion,
            controller: 'CardsCtrl',
            controllerAs: 'vm'
        })
        .state('cards.add', {
            url: '/card/add',
            parent: 'dashboard',
            templateUrl: 'views/pages/dashboard/cards/add-card.html?v='+window.appVersion,
            controller: 'CardsCtrl',
            controllerAs: 'vm'
        })
        .state('cards.edit', {
            ...
            parent: 'dashboard',
            templateUrl: 'views/pages/dashboard/cards/edit-card.html?v='+window.appVersion,
            controller: 'CardsCtrl',
            controllerAs: 'vm'
        });
	});
	...
	CardsCtrl.prototype.getCards = function(){
		...
		vm.CardsService.getCards(vm.licensePlateSearch, vm.makeSearch, vm.modelSearch)
		.then(function() {
	...
	CardsCtrl.prototype.getCard = function(){
		...
		.then(function() {
			...
			alert(vm.$filter('translate')('getCardFailed') + ' Message from server: ' + reason);
			vm.$log.log(reason);
		});
	};
	...
	CardsCtrl.prototype.initAddCard = function(){
		...
		.then(function() {
			...
			alert(vm.$filter('translate')('getFormDataFailed') + ' Message from server: ' + reason);
		});
	};
	...
	CardsCtrl.prototype.initEditCard = function(){
		...
		.then(function() {
			...
			if(vm.card.vin) {
				...
				vm.models = vm.$filter('filter')(vm.makes, {name: vm.card.make}, true)[0].models;
				vm.model = vm.$filter('filter')(vm.models, {name: vm.card.model}, true)[0];
				if (!vm.model) {
